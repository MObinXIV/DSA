...
{
    if(j<0 || j > m) return 1e9;//out of boundary case 
    if(i==0) return matrix[0][j];//any element in i
    if(dp[i][j]!=-1) return dp[i][j];
    int up=matrix[i][j]+f(i-1,j,m,matrix,dp);
    int Ldig=matrix[i][j]+f(i-1,j-1,m,matrix,dp);
    int Rdig=matrix[i][j]+f(i-1,j+1,m,matrix,dp);
    return dp[i][j]=min(up,min(Ldig,Rdig));
